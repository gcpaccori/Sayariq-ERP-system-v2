...
function parseData(raw: string | undefined) {
  if (!raw) return null;
  try {
...
export default async function ComprobantePage({
  ...
  const tipo = String(search.tipo ?? "").toLowerCase();
  const data = parseData(search.data);
  ...
  const pairs = Object.entries(data)
    ...
    .map(([k, v]) => {
      if (
        typeof v === "number" &&
        (k.includes("monto") || k.includes("total") || k.includes("precio"))
      ) {
  ...
  return (
    <main className="mx-auto max-w-4xl p-6 print:p-0">
      <div className="mb-4 flex items-center justify-between print:hidden">
        <Link href="/liquidaciones" className="rounded border px-3 py-2 text-sm">
          ← Volver
        </Link>
      ...
      <section className="rounded-xl border bg-white p-6 shadow-sm print:rounded-none print:border-0 print:shadow-none">
        <div className="mb-6 border-b pb-4">
          ...
          <p className="text-sm text-gray-600">
            Código interno / referencia: <strong>{codigo}</strong>
          </p>
        </div>
