      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl p-6">
        ...
        <section className="mb-8 grid gap-4 sm:grid-cols-4">
          {[
            ...
            { label: "Kg en almacén", value: resumen.kgAlmacen, color: "from-purple-50 to-purple-50", textColor: "text-purple-700", icon: "⚖️" },
          ].map((card) => (
        ...
        <section className="mb-6">
          ...
          <form action={clasificarLoteAction} className="grid gap-3">
            ...
            <label className="grid gap-1 max-w-xs">
              <span className="text-sm font-medium text-gray-700">Fecha clasificación *</span>
              <input
            ...
            <p className="text-xs text-gray-500">Qué muestra esta tabla: formato de captura por categoría para registrar la clasificación del lote.</p>
            ...
            <div className="flex gap-2">
              <button type="submit" className="inline-flex items-center gap-2 rounded-md bg-green-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-green-700">
                Guardar clasificación
              </button>
              <Link href="/almacen" className="inline-flex items-center gap-2 rounded-md border border-gray-200 bg-white px-4 py-2 text-sm font-medium shadow-sm hover:bg-gray-50">
                Cancelar
              </Link>
            </div>
      ...
      {loteVerDetalle ? (
        <section className="mb-6 rounded-lg border border-gray-100 bg-white p-4 shadow-sm">
          ...
          <p className="mb-2 text-sm text-gray-700">
            Total bruto clasificado: {Math.round(totalBrutoClasificado * 100) / 100} kg | Total neto clasificado: {Math.round(totalNetoClasificado * 100) / 100} kg
          </p>
          <p className="mb-3 text-sm text-gray-700">Diferencia: {detalleDiferencia} kg | Merma: {detalleMerma}%</p>
          {fotoClasificacionDetalle ? (
            <div className="mb-3">
              <p className="mb-1 text-xs text-gray-500">Miniatura correlacionada: evidencia de clasificación</p>
          ...
          <p className="text-xs text-gray-500">Qué muestra esta tabla: resultado técnico de clasificación (bruto, descuentos y neto) por categoría.</p>
          <div className="overflow-x-auto rounded-lg border border-gray-100">
            <table className="min-w-full border-collapse text-sm">
              <thead>
                <tr className="border-b text-left">
